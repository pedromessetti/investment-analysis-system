...
def download_xlsx():
    ...
    if get_url == url:
        button = wait.until(common.EC.visibility_of_element_located((common.By.ID, 'botao_arquivo')))
    button.click()
    ...
    file_name = next(filename for filename in common.os.listdir(downloads_folder) if filename.endswith('.xlsx') and filename.startswith('Stock_Screener_'))

    # Move the file from Downloads folder to the current folder
    common.shutil.move(common.os.path.join(downloads_folder, file_name), 'csv/invest_site.xlsx')
...
def convert_to_csv():
    ...
    # Open the XLSX file
    workbook = common.openpyxl.load_workbook('csv/invest_site.xlsx')
    ...
    with open('csv/invest_site.csv', 'w', newline='') as csvfile:
        writer = common.csv.writer(csvfile)
